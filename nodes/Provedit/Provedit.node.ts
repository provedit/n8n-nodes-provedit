...
export class Provedit implements INodeType {
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      try {
        if (operation === 'decide') {
          const params = parseJson(this.getNodeParameter('params', i, '{}'), 'Params');
        ...
        const params = parseJson(this.getNodeParameter('paramsRecord', i, '{}'), 'Params');
...
function parseJson(value: unknown, label: string): Record<string, unknown> {
  ...
  try {
    ...
    throw new ApplicationError(`${label} is not valid JSON: ${(err as Error).message}`);
  }
}
